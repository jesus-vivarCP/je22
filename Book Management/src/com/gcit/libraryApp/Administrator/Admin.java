...
public class Admin implements Administrator {
	...
	public void Add(String table, Map<String, ArrayList<String>> values, Map<String, ArrayList<String>> columnTags) {
		...
		if (tables[0].equals("tbl_book")) {
			addAuthor(ids.get(0), ids.get(1));
		}
		ids.clear();
	}
	...
	private void addAuthor(int bookId, int authorId) {
		...
		try {
			...
			if (conn != null)
				try {
					gf.println("Rolled back successful");
					conn.rollback();
				} catch (SQLException e1) {
					gf.println("Fail to rollback");
					/* Do nothing */}
	...
	private void setTo(int param, String s, PreparedStatement p) {
		try {
			try {
				p.setInt(param, Integer.parseInt(s));
			} catch (NumberFormatException e) {
				p.setString(param, s);
			}
		} catch (SQLException e) {
			gf.println("Exception on preparing the query.");
		}
	}
	...
	public int Update(String table, ArrayList<String> columns, ArrayList<String> values) {
		...
		try {
			...
			for (idx = 1; idx < size; idx++) {
				setTo(idx, values.get(idx), pstmt);
				gf.println(values.get(idx));
			}
			pstmt.setInt(idx, id);
			pstmt.executeUpdate();
			conn.commit();
			...
			gf.println("Update of table " + table + " failed.");
		}
	...
	public void Delete(String table, ArrayList<String> columns, ArrayList<String> values) {
		...
		try {
			PreparedStatement pstmt = conn.prepareStatement(query);
			// for(idx=1;idx<size;idx++)
			// {setTo(idx,values.get(idx),pstmt);gf.println(values.get(idx));}
			pstmt.setInt(1, id);
			pstmt.executeUpdate();
			conn.commit();
		} catch (SQLException e) {
			gf.println(nameTag + ": does not exists.");
		}
	}

	@Override
	public void getColumnNames(String table, Map<String, ArrayList<String>> args,
	...
	private void getTableColumns(String table, ArrayList<String> args) {
		...
		try {
			...
			gf.println("Getting columns name failed.");
		}
	}
	...
	public void Override(String table, ArrayList<String> columnTags, ArrayList<String> values) {
		...
		if (values.get(size - 1) != null) {
			gf.println("Records show book has been turn.");
			return;
		}

		size -= 2;
		query = "Update " + table + " Set dueDate = DATE_ADD(NOW(),INTERVAL 7 DAY),dateIn=null Where ";

		for (idx = 0; idx < size; idx++) {
			query += columnTags.get(idx) + " = ? and ";
			gf.println(columnTags.get(idx));
		}
	...
	private void operation(String task, String table, ArrayList<String> columnTags, ArrayList<String> values) {
		switch (task) {
		case "OVERRIDE":
			gf.println("Overriding...");
			Override(table, columnTags, values);
			break;
		case "UPDATE":
			gf.println("Updating...");
			break;
		case "DELETE":
			gf.println("Deleting...");
			Delete(table, columnTags, values);
			break;
		default:
			gf.println(task + ": Not an option.");
			break;
		}
	}

	@Override
	public ArrayList<String> taskAtTableLocation(Connection conn, String task, ArrayList<String> required,
